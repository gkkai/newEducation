...
public class ColumnActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        onCreateView(R.layout.activity_column);
    }
    @Override
...
//        }
    ...
    private void initView(final PaperOneLevelTypeListBean bean) {
...
//                }
                ...
                new NewPreferManager().saveColumbean(title,gson.toJson(bean));
...
//                datas.remove(position);
    ...
    protected void initComponent() {
        ...
        map.put("pid",pid);
        OkhttpUtils.getInstance(this).getList("academyService/examinationPaperType/findExaminationPaperSubTypeList", map, new OkhttpUtils.onCallBack() {
            @Override
            ...
            public void onResponse(final String json) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            PaperOneLevelTypeListBean bean= gson.fromJson(json,PaperOneLevelTypeListBean.class);
...
//                                        PaperOneLevelTypeListBean bean1=gson.fromJson(NewPreferManager.getIntelligence(),PaperOneLevelTypeListBean.class);
//                                        //TODO  先清除
...
//                                        PaperOneLevelTypeListBean bean1=gson.fromJson(NewPreferManager.getWeekColumbean(),PaperOneLevelTypeListBean.class);
//                                        //TODO  先清除
//                                        for(int i=0;i<bean1.getData().size();i++){
//
//                                            if(iscontains(bean,bean1.getData().get(i))){
...
//
//                                    }
                                    if(!NewPreferManager.geteColumbean(title).equals("")){
                                        PaperOneLevelTypeListBean bean1=gson.fromJson(NewPreferManager.geteColumbean(title),PaperOneLevelTypeListBean.class);
                                        //TODO  先清除
                                        try {
                                            for(int i=0;i<bean1.getData().size();i++){
    ...
    private boolean iscontains(PaperOneLevelTypeListBean bean,PaperOneLevelTypeListBean.Data bean1){
        for(int i=0;i<bean.getData().size();i++){
...
//       将属性设置给窗体
    ...
    public class MsgContentFragmentAdapter extends FragmentPagerAdapter {
        private List<PaperOneLevelTypeListBean.Data> names;
         ...
         * @param datas
         */
        public void setList(List<PaperOneLevelTypeListBean.Data> datas) {
            this.names.clear();
