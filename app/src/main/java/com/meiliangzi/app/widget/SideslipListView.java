...
public class SideslipListView extends ListView {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {//事件拦截
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (Math.abs(diffX) > 20 || Math.abs(nowY - mDownY) > 20) {
    ...
    private boolean performActionMove(MotionEvent ev) {
        if (mPointChild != null) {
            ...
            /*if (Math.abs(diffX) > Math.abs(nowY - mDownY) && Math.abs(nowY - mDownY) < 10) {
           ...
           int lenth= Math.abs(diffX);
            int hight= Math.abs(diffY);
            if (lenth > 50 ) {
                if (!isDeleteShow && nowX < mDownX) {//删除按钮未显示时向左滑
                    if (-diffX >= mDeleteWidth) {//如果滑动距离大于删除组件的宽度时进行偏移的最大处理
                        diffX = -mDeleteWidth;
